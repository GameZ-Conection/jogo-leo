  ...
  return (
    <AuthGuard>
      <main className="min-h-screen bg-gradient-to-br from-slate-950 via-blue-950 to-slate-900 text-slate-100 overflow-hidden">
        ...
        <div className="mt-10 mr-10 absolute z-11 right-0 top-0">
          <button
            className="text-slate-300 hover:text-white underline text-sm mt-2 flex flex-row items-center gap-2 cursor-pointer"
            onClick={() => logout()}
          >
            <p className="font-bold tracking-wide">Sair</p>
            <span>
              <img src="/icon-sair/icon-sair.png" alt="" className="w-10 h-10" />
            </span>
          </button>
        ...
        <div className="relative z-10 flex flex-col items-center justify-center min-h-screen px-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6 mb-12 max-w-2xl w-full">
            ...
            <button
              ...
              className={`group card button-hover p-8 text-center transition-all duration-700 transform cursor-pointer ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-8'
                }`}
          ...
          {/* Dica */}
          <div className={`glass rounded-lg p-4 max-w-md text-center transition-all duration-700 ${isLoaded ? 'opacity-100' : 'opacity-0'
            }`}
          ...
          {/* Rodapé */}
          <div className={`mt-16 text-center text-slate-500 text-xs transition-all duration-700 ${isLoaded ? 'opacity-100' : 'opacity-0'
            }`}
